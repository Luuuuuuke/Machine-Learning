...
def neuralNetworkTraining_ope():
    ...
    with open(traininglabels_path) as file:
        ...
        for row in file_dict:
            dict_id_ope[row['userid']] = float(row['ope'])
            count += 1
            pass

    #for each traning example
    flag = 0
    for key,val in dict_id_ope.items():
        userid = key
        ope = val
        ...
        delta_w_2 = [0 for col in range(N_HIDDEN)]
        for j in range(0, N_HIDDEN, 1):
            delta_w_2[j] = LEARNING_RATE * (target_ope - output) * output * (1 - output) * output_hidden[j] + MOMENTUM * delta_w_2[j]
        #update the weight
        ...
        error_term_output = output * (1 - output) * (target_ope - output)
